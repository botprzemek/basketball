...
use api_domain::{entities::Role, ports::RolePort};
...
#[diesel(table_name = crate::repositories::roles)]
...
impl RolePort for RoleRepository {
    async fn select(&self, self_organization_id: Uuid) -> anyhow::Result<Vec<Role>> {
    ...
    async fn select_by_identity(
        ...
        let result = rows.into_iter().map(Role::from).collect::<Vec<_>>();
